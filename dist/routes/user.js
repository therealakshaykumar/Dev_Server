...
ROUTER.patch('/profile', upload.single('image'), asyncHandler((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (req.file) {
        ...
        if (currentUser === null || currentUser === void 0 ? void 0 : currentUser.imagePublicId) {
            yield deleteFromCloudinary(currentUser.imagePublicId);
        }
        const { url, publicId } = yield uploadToCloudinary(req.file.buffer);
        updateData.imageUrl = url;
        updateData.imagePublicId = publicId;
    }
    const USER = yield User.findByIdAndUpdate(req.userId, updateData, { new: true }).select("-password -__v");
    if (!USER) {
        return res.status(404).send("User not found");
    }
    res.json(USER);
